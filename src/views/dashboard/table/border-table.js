  ...
  const columns = useRef([
    {
      ...
      data: 'user',
      render: function (row) {
        return `<div class="d-flex align-items-center"><img class="rounded img-fluid  me-3" src="${row.img}" alt=""/><div class="media-support-info"><h5 class="mb-0">${row.name}</h5><p class="mb-0">${row.desc}</p></div></div>`
      }
    },
    {
      title: 'Contact',
      ...
      render: function (value) {
        return `<span class='text-dark'>${value.no}</span>`
      }
    },
    {
      title: 'Email ID',
      data: 'emailid',
      render: function (row) {
        return `<span class='text-dark'>${row.id}</span>`
      }
    },
    {
      title: 'Country',
      data: 'country',
      render: function (row) {
        return `<span class='me-2'>${row.countrysvg}</span><span class='text-dark'>${row.countryname}</span>`
      }
    },
    {
      title: 'Purchases',
      data: 'purchases',
      render: function (row) {
        return `<span class='text-dark'>${row.purchases}</span>`
      }
    },
    {
      title: 'Status',
      data: 'statustype',
      render: function (row) {
        return `<span class="badge bg-${row.bgColor}-subtle p-2 text-${row.statusColor}">${row.status}</span>`
      }
    },
    {
      title: 'Action',
      data: 'action',
      render: function (row) {
        return `<div class="d-flex gap-3 justify-content-center"><a class="btn btn-primary btn-icon btn-sm rounded-pill" href="/blog/blog-category" role="button"><span class="btn-inner">${row.actionadd}</span></a > <div class="d-flex gap-3 justify-content-center"><a class="btn btn-primary btn-icon btn-sm rounded-pill" href="/blog/blog-category" role="button"><span class="btn-inner">${row.actionedit}</span></a><div class="d-flex gap-3 justify-content-center"><a class="btn btn-primary btn-icon btn-sm rounded-pill" href="/blog/blog-category" role="button"><span class="btn-inner">${row.actionremove}</span></a>`
      }
    }
  ])
